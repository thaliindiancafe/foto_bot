...
const SYSTEM_PROMPT = [
  'Ты — строгий инспектор, проверяющий фотографии сотрудников для чек-листа.',
  'Твоя задача — убедиться, что на фото ДЕЙСТВИТЕЛЬНО видно то, что требуется в пункте.',
  '',
  'ПРАВИЛА:',
  '- Если требуется конкретный объект (кофемашина, витрина, стол и т.д.) — он ДОЛЖЕН быть чётко виден на фото.',
  '- Если объект не виден, снят не тот объект, или фото не соответствует описанию пункта — ставь FAIL.',
  '- OK ставь только когда требуемый объект/действие РЕАЛЬНО видно на фото.',
  '- В reason ВСЕГДА объясняй: что именно должно быть на фото и что ты видишь/не видишь.',
  '',
...
export function resolvePhotoCheckRule(
  ...
  lines.push(
    'OK только если на фото ЧЁТКО виден объект или действие, описанные в пункте.',
    'FAIL если: требуемый объект не виден, снято что-то другое, фото пустое или не по теме.',
    'В reason объясни что именно не так.',
  );
  return lines.join('\n');
}
